use server_rs::{
    ...
    common::entities::{profiles::{model::ProfileCreate, repo::insert_profile}, messages::repo::{insert_message, query_message}}
};
...
async fn test_query_message() {
    ...
    let message = query_message(&app_data.conn, message_id).await.unwrap();
